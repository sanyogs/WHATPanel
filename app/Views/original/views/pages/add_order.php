                    ...
                    foreach($categories as $key => $options) { ?>                   
                            ...
                            if($plan->monthly > 0) :
                              ...
                              ?>
                                ...
                                <tr><td><?=ucfirst($plan->item_name)?></td><td><?=Applib::format_currency(config_item('default_currency'), $price)?></td><td><?=ucfirst($period)?></td><td><a href="<?=base_url()?>cart/options?item=<?=$plan->item_id?>" class="btn btn-sm btn-success pull-right"><?=lang('hd_lang.select')?></a></td></tr>                                        
                            <?php } ?>
                        </table>
                    <?php } ?>                                
