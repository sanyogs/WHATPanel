...
<script src="<?=base_url()?>js/slider/bootstrap-slider.js"></script>
<script src="<?=base_url()?>js/bootstrap-datepicker/js/bootstrap-datepicker.min.js"></script>
<script src="<?=base_url()?>js/bootstrap-datepicker/locales/bootstrap-datepicker.<?=(lang('hd_lang.lang_code') == 'en' ? 'en-GB': lang('hd_lang.lang_code'))?>.min.js"></script>
...
<?php if (isset($form)) { ?>
<script src="<?=base_url()?>js/file-input/bootstrap-filestyle.min.js"></script>
<script src="<?=base_url()?>js/parsley/parsley.min.js"></script>
<script src="<?=base_url()?>js/parsley/parsley.extend.js"></script>
<?php } ?>

<?php
if (isset($datatables)) {
    $sort = strtoupper($custom_helper->getconfig_item('date_picker_format'));
?>
...
<script src="<?=base_url()?>js/datatables/datetime-moment.js"></script>
<script type="text/javascript">
(function($){
"use strict"; 
        ...
        "iDisplayLength": <?=$custom_helper->getconfig_item('rows_per_table')?>,
        "oLanguage": {
                "sProcessing": "<?=lang('hd_lang.processing')?>",
                "sLoadingRecords": "<?=lang('hd_lang.loading')?>",
                "sLengthMenu": "<?=lang('hd_lang.show_entries')?>",
                "sEmptyTable": "<?=lang('hd_lang.empty_table')?>",
                "sInfo": "<?=lang('hd_lang.pagination_info')?>",
                "sInfoEmpty": "<?=lang('hd_lang.pagination_empty')?>",
                "sInfoFiltered": "<?=lang('hd_lang.pagination_filtered')?>",
                "sInfoPostFix":  "",
                "sSearch": "<?=lang('hd_lang.search')?>:",
                "sUrl": "",
                "oPaginate": {
                        "sFirst":"<?=lang('hd_lang.first')?>",
                        "sPrevious": "<?=lang('hd_lang.previous')?>",
                        "sNext": "<?=lang('hd_lang.next')?>",
                        "sLast": "<?=lang('hd_lang.last')?>"
                }
        },
        "tableTools": {
                    "sSwfPath": "<?=base_url()?>js/datatables/tableTools/swf/copy_csv_xls_pdf.swf",
              "aButtons": [
                      {
                      "sExtends": "csv",
                      "sTitle": "<?=$custom_helper->getconfig_item('company_name').' - '.lang('hd_lang.invoices')?>"
                  },
                      {
                      "sExtends": "xls",
                      "sTitle": "<?=$custom_helper->getconfig_item('company_name').' - '.lang('hd_lang.invoices')?>"
        ...
        $('#table-translations').on('click','.backup-translation', function (e) {
            ...
            $.ajax({
                ...
                data: {},
                success: function() {
                    toastr.success("<?=lang('hd_lang.operation_successful')?>", "<?=lang('hd_lang.response_status')?>");
                },
                error: function(xhr) {
                    alert('Error: '+JSON.stringify(xhr));
                }
        ...
        $("#table-translations").on('click', '.restore-translation', function (e) {
            ...
            $.ajax({
                ...
                data: {},
                success: function() {
                    toastr.success("<?=lang('hd_lang.translation_restored_successfully')?>", "<?=lang('hd_lang.response_status')?>");
                },
                error: function(xhr) {
                    alert('Error: '+JSON.stringify(xhr));
                }
        ...
        $('#table-translations').on('click','.submit-translation', function (e) {
            ...
            $.ajax({
                ...
                data: {},
                success: function() {
                    toastr.success("<?=lang('hd_lang.translation_submitted_successfully')?>", "<?=lang('hd_lang.response_status')?>");
                },
                error: function(xhr) {
                    alert('Error: '+JSON.stringify(xhr));
                }
...
<script type="text/javascript" src="<?=base_url()?>js/iconpicker/fontawesome-iconpicker.min.js"></script>
<script type="text/javascript">
(function($){
"use strict";
    $(document).ready(function () {
...
<script type="text/javascript" src="<?=base_url()?>js/sortable/jquery-sortable.js"></script>
<script type="text/javascript">
    var t1, t2, t3, t4, t5;
...
$last = service('uri')->getTotalSegments(); 
if (service('uri')->getSegment(1) != 'accounts' 
&& service('uri')->getSegment(1) != 'domains' 
&& service('uri')->getSegment(1) != 'orders' 
|| (null != service('uri')->getSegment(2) && service('uri')->getSegment(2) == 'add_order')) {
if($session->getFlashdata('message')){
$message = $session->getFlashdata('message');
$alert = $session->getFlashdata('response_status'); ?>
<script type="text/javascript">
(function($){
"use strict"; 
    $(document).ready(function(){
...
<?php if (isset($menus)) { ?>
    ...
    <script type="text/javascript" src="<?=base_url()?>js/nestedSortable/jquery.mjs.nestedSortable.js"></script>
    ...
    <script type="text/javascript">             
        ...
        $("#form-menu").on('click','.activate-item',function (e) {
            ...
            $.ajax({
                ...
                data: { active: isActive },
                success: function() {
                    toastr.success("<?=lang('hd_lang.menu_item_status')?>", "<?=lang('hd_lang.response_status')?>");
                },
                error: function(xhr) {
                    alert('Error: '+JSON.stringify(xhr));
                }
