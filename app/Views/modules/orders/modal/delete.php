...
<div class="modal-dialog modal-lg my-modal modal-cus">
	<div class="modal-content">
		...
		<input type="hidden" name="invoice_id" value="<?=$order[0]->invoice_id?>">
			...
			<div class="row"> 
				<div class="col-md-12">
				<span class="pull-right common-p d-flex align-items-center gap-3"><?=lang('hd_lang.credit_account')?>: <label class="switch">
		...
		<button <?=($custom->getconfig_item('demo_mode') == 'TRUE') ? 'disabled' : ''?> style='border:none;' type="submit" class="btn btn-<?=$custom->getconfig_item('theme_color');?>"><?=lang('hd_lang.delete_order')?></button>
		</form>
		</div>
	</div>
	<!-- /.modal-content -->
