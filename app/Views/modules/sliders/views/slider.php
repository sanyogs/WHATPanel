<?php 
/*
 * This file is part of WHATPANEL.
 *
 * @package     WHAT PANEL – Web Hosting Access Terminal Panel.
 * @copyright   2023-2024 Version Next Technologies and MadPopo. All rights reserved.
 * @license     BSL; see LICENSE.txt
 * @link        https://www.version-next.com
 */

use App\Helpers\custom_name_helper;

$custom_name_helper = new custom_name_helper();
?>
<?= $this->extend('layouts/users') ?>

<?= $this->section('content') ?>

<!-- menu -started -->
<section id="slider-wrap">
    <div class="container px-0">
        <div class="slider-topbar-wrap">
            <div class="slider-title-wrap">
                <h3>Sliders</h3>
                <p>Add sliders</p>
            </div>
            <div class="slider-last-wrap">
				<form method='get' action="<?php echo base_url('sliders/slider/'. $slider_id); ?>" id="searchForm">
					<input type="text" name="search" placeholder="Search for Code" />
					<a href="<?php echo base_url('sliders/slider/'. $slider_id); ?>" class="btn new-hosting-div bg-danger clrBtn">Clear</a>
				</form>
                <a href="<?= base_url('sliders/add_slide/' . $slider_id) ?>" class="slider-add-item-div" data-toggle="ajaxModal">
                    <span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                            <path d="M10.6667 10.3333C10.6667 10.5101 10.5964 10.6797 10.4714 10.8047C10.3464 10.9298 10.1768 11 10 11H8.66667V12.3333C8.66667 12.5101 8.59643 12.6797 8.47141 12.8047C8.34638 12.9298 8.17681 13 8 13C7.82319 13 7.65362 12.9298 7.5286 12.8047C7.40357 12.6797 7.33333 12.5101 7.33333 12.3333V11H6C5.82319 11 5.65362 10.9298 5.5286 10.8047C5.40357 10.6797 5.33333 10.5101 5.33333 10.3333C5.33333 10.1565 5.40357 9.98695 5.5286 9.86193C5.65362 9.73691 5.82319 9.66667 6 9.66667H7.33333V8.33333C7.33333 8.15652 7.40357 7.98695 7.5286 7.86193C7.65362 7.73691 7.82319 7.66667 8 7.66667C8.17681 7.66667 8.34638 7.73691 8.47141 7.86193C8.59643 7.98695 8.66667 8.15652 8.66667 8.33333V9.66667H10C10.1768 9.66667 10.3464 9.73691 10.4714 9.86193C10.5964 9.98695 10.6667 10.1565 10.6667 10.3333ZM16 5.66667V12.3333C15.9989 13.2171 15.6474 14.0643 15.0225 14.6892C14.3976 15.3141 13.5504 15.6656 12.6667 15.6667H3.33333C2.4496 15.6656 1.60237 15.3141 0.97748 14.6892C0.352588 14.0643 0.00105857 13.2171 0 12.3333L0 4.33333C0.00105857 3.4496 0.352588 2.60237 0.97748 1.97748C1.60237 1.35259 2.4496 1.00106 3.33333 1H5.01867C5.32893 1.00026 5.63493 1.07236 5.91267 1.21067L8.01667 2.26667C8.10963 2.31128 8.21155 2.33408 8.31467 2.33333H12.6667C13.5504 2.33439 14.3976 2.68592 15.0225 3.31081C15.6474 3.93571 15.9989 4.78294 16 5.66667ZM1.33333 4.33333V5H14.544C14.4066 4.61139 14.1525 4.27473 13.8165 4.03606C13.4804 3.79739 13.0788 3.66838 12.6667 3.66667H8.31467C8.0044 3.66641 7.6984 3.5943 7.42067 3.456L5.31667 2.40333C5.22398 2.35757 5.12204 2.33362 5.01867 2.33333H3.33333C2.8029 2.33333 2.29419 2.54405 1.91912 2.91912C1.54405 3.29419 1.33333 3.8029 1.33333 4.33333ZM14.6667 12.3333V6.33333H1.33333V12.3333C1.33333 12.8638 1.54405 13.3725 1.91912 13.7475C2.29419 14.1226 2.8029 14.3333 3.33333 14.3333H12.6667C13.1971 14.3333 13.7058 14.1226 14.0809 13.7475C14.456 13.3725 14.6667 12.8638 14.6667 12.3333Z" fill="white" />
                        </svg>
                    </span>
                    <p>Add Sliders</p>
                </a>
            </div>
        </div>

        <div class="slider-table-wrap">
            <div class="sliderTableInfoHead">
                <div class="sliderShowEntriesWrap">
                    <span>Show</span>
                    <form action="<?php echo base_url('sliders/slider/' . $slider_id); ?>" method="get">
						<select name="recordsPerPage" onchange="this.form.submit()">
							
						</select>
					</form>
                    <span>Entries</span>
                </div>
            </div>
            <div class="slider-table-overflow">
                <table class="slider-table">
                    <tr>
                        <th>Slider</th>
                        <th>Title</th>
                        <th>Descriptions</th>
                        <th>Action</th>
                    </tr>
                    <?php foreach ($slider as $key => $row) { ?>
                        <tr>
                            <td>
                                <?php if (!empty($row->image)) { ?>
                                    <img src="<?= base_url('uploads/' . $row->image) ?>" class="list_thumb" />
                                <?php } ?>
                            </td>

                            <td>
                                <?= $row->title ?>
                            </td>

                            <td>
                                <?= $row->description ?>
                            </td>
                            <td>
                                <div>
                                    <a href="<?= base_url('sliders/edit_slide/' . $row->slide_id) ?>" data-toggle="ajaxModal">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                            <g clip-path="url(#clip0_1748_541)">
                                                <path d="M12.437 0.619885L4.30902 8.74789C3.99858 9.05665 3.75247 9.42392 3.58494 9.82845C3.41741 10.233 3.33178 10.6667 3.33302 11.1046V11.9999C3.33302 12.1767 3.40326 12.3463 3.52828 12.4713C3.65331 12.5963 3.82288 12.6666 3.99969 12.6666H4.89502C5.33286 12.6678 5.7666 12.5822 6.17113 12.4146C6.57565 12.2471 6.94292 12.001 7.25169 11.6906L15.3797 3.56255C15.7693 3.172 15.9881 2.64287 15.9881 2.09122C15.9881 1.53957 15.7693 1.01044 15.3797 0.619885C14.9835 0.241148 14.4565 0.0297852 13.9084 0.0297852C13.3602 0.0297852 12.8332 0.241148 12.437 0.619885ZM14.437 2.61989L6.30902 10.7479C5.9331 11.1215 5.42503 11.3318 4.89502 11.3332H4.66635V11.1046C4.66774 10.5745 4.87807 10.0665 5.25169 9.69055L13.3797 1.56255C13.5221 1.42652 13.7114 1.35061 13.9084 1.35061C14.1053 1.35061 14.2946 1.42652 14.437 1.56255C14.577 1.7029 14.6556 1.89301 14.6556 2.09122C14.6556 2.28942 14.577 2.47954 14.437 2.61989Z" fill="#2E0AA3" />
                                                <path d="M15.3333 5.986C15.1565 5.986 14.987 6.05624 14.8619 6.18126C14.7369 6.30629 14.6667 6.47586 14.6667 6.65267V10H12C11.4696 10 10.9609 10.2107 10.5858 10.5858C10.2107 10.9609 10 11.4696 10 12V14.6667H3.33333C2.8029 14.6667 2.29419 14.456 1.91912 14.0809C1.54405 13.7058 1.33333 13.1971 1.33333 12.6667V3.33333C1.33333 2.8029 1.54405 2.29419 1.91912 1.91912C2.29419 1.54405 2.8029 1.33333 3.33333 1.33333H9.36133C9.53815 1.33333 9.70771 1.2631 9.83274 1.13807C9.95776 1.01305 10.028 0.843478 10.028 0.666667C10.028 0.489856 9.95776 0.320286 9.83274 0.195262C9.70771 0.0702379 9.53815 0 9.36133 0L3.33333 0C2.4496 0.00105857 1.60237 0.352588 0.97748 0.97748C0.352588 1.60237 0.00105857 2.4496 0 3.33333L0 12.6667C0.00105857 13.5504 0.352588 14.3976 0.97748 15.0225C1.60237 15.6474 2.4496 15.9989 3.33333 16H10.8953C11.3333 16.0013 11.7671 15.9156 12.1718 15.7481C12.5764 15.5806 12.9438 15.3345 13.2527 15.024L15.0233 13.252C15.3338 12.9432 15.58 12.576 15.7477 12.1715C15.9153 11.767 16.0011 11.3332 16 10.8953V6.65267C16 6.47586 15.9298 6.30629 15.8047 6.18126C15.6797 6.05624 15.5101 5.986 15.3333 5.986ZM12.31 14.0813C12.042 14.3487 11.7031 14.5337 11.3333 14.6147V12C11.3333 11.8232 11.4036 11.6536 11.5286 11.5286C11.6536 11.4036 11.8232 11.3333 12 11.3333H14.6167C14.5342 11.7023 14.3493 12.0406 14.0833 12.3093L12.31 14.0813Z" fill="#2E0AA3" />
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_1748_541">
                                                    <rect width="16" height="16" fill="white" />
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </a>

                                    <a href="<?= base_url('sliders/delete_slide/' . $row->slide_id) ?>" data-toggle="ajaxModal">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                            <g clip-path="url(#clip0_1748_550)">
                                                <path d="M13.9997 2.66667H11.933C11.7783 1.91428 11.3689 1.23823 10.7738 0.752479C10.1788 0.266727 9.43448 0.000969683 8.66634 0L7.33301 0C6.56487 0.000969683 5.82055 0.266727 5.22551 0.752479C4.63046 1.23823 4.22107 1.91428 4.06634 2.66667H1.99967C1.82286 2.66667 1.65329 2.7369 1.52827 2.86193C1.40325 2.98695 1.33301 3.15652 1.33301 3.33333C1.33301 3.51014 1.40325 3.67971 1.52827 3.80474C1.65329 3.92976 1.82286 4 1.99967 4H2.66634V12.6667C2.6674 13.5504 3.01893 14.3976 3.64382 15.0225C4.26871 15.6474 5.11594 15.9989 5.99967 16H9.99968C10.8834 15.9989 11.7306 15.6474 12.3555 15.0225C12.9804 14.3976 13.332 13.5504 13.333 12.6667V4H13.9997C14.1765 4 14.3461 3.92976 14.4711 3.80474C14.5961 3.67971 14.6663 3.51014 14.6663 3.33333C14.6663 3.15652 14.5961 2.98695 14.4711 2.86193C14.3461 2.7369 14.1765 2.66667 13.9997 2.66667ZM7.33301 1.33333H8.66634C9.07986 1.33384 9.48309 1.46225 9.82075 1.70096C10.1584 1.93967 10.414 2.27699 10.5523 2.66667H5.44701C5.5854 2.27699 5.84094 1.93967 6.1786 1.70096C6.51626 1.46225 6.91949 1.33384 7.33301 1.33333ZM11.9997 12.6667C11.9997 13.1971 11.789 13.7058 11.4139 14.0809C11.0388 14.456 10.5301 14.6667 9.99968 14.6667H5.99967C5.46924 14.6667 4.96053 14.456 4.58546 14.0809C4.21039 13.7058 3.99967 13.1971 3.99967 12.6667V4H11.9997V12.6667Z" fill="#CC0000" />
                                                <path d="M6.66667 11.9998C6.84348 11.9998 7.01305 11.9296 7.13807 11.8046C7.2631 11.6796 7.33333 11.51 7.33333 11.3332V7.33317C7.33333 7.15636 7.2631 6.98679 7.13807 6.86177C7.01305 6.73674 6.84348 6.6665 6.66667 6.6665C6.48986 6.6665 6.32029 6.73674 6.19526 6.86177C6.07024 6.98679 6 7.15636 6 7.33317V11.3332C6 11.51 6.07024 11.6796 6.19526 11.8046C6.32029 11.9296 6.48986 11.9998 6.66667 11.9998Z" fill="#CC0000" />
                                                <path d="M9.33366 11.9998C9.51047 11.9998 9.68004 11.9296 9.80506 11.8046C9.93009 11.6796 10.0003 11.51 10.0003 11.3332V7.33317C10.0003 7.15636 9.93009 6.98679 9.80506 6.86177C9.68004 6.73674 9.51047 6.6665 9.33366 6.6665C9.15685 6.6665 8.98728 6.73674 8.86225 6.86177C8.73723 6.98679 8.66699 7.15636 8.66699 7.33317V11.3332C8.66699 11.51 8.73723 11.6796 8.86225 11.8046C8.98728 11.9296 9.15685 11.9998 9.33366 11.9998Z" fill="#CC0000" />
                                            </g>
                                            <defs>
                                                <clipPath id="clip0_1748_550">
                                                    <rect width="16" height="16" fill="white" />
                                                </clipPath>
                                            </defs>
                                        </svg>
                                    </a>
                                </div>
                            </td>
                        </tr>
                    <?php } ?>
                </table>
            </div>
            <div class="slider-table-pagination">
                <div class="showingEntriesWrap">
				  <?php
				  //$totalItems = $pager->getTotal(); // Get total number of items from the pager
				 // $currentStart = ($pager->getCurrentPage() - 1) * $perPage + 1; // Calculate the start index of the current page
				  //$currentEnd = min($currentStart + $perPage - 1, $totalItems); // Calculate the end index of the current page

				 // $showEntry = "Showing $currentStart to $currentEnd of $totalItems entries";

				  ?>
				  <p><?//= $showEntry; ?></p>
			  </div>
                <div class="slider-pagination-wrap">
                    <ul class="slider-pagination">
                        <div class="row">
						  <?php if (!empty($servers)) : ?>
							<!-- If there are items, display the pagination links -->
							<?php if ($pager) : ?>
							  <ul class="pagination">
								<?php
								$pager->setPath('sliders/slider/'. $slider_id);

								// Output Pagination Links
								echo $pager->links();
								?>
							  </ul>
							<?php endif; ?>

						  <?php else : ?>
							<!-- If there are no items, display the message -->
							<div class="col-12 text-center">
							  <h1 class="text-center"><?//= esc($message) ?></h1>
							</div>
						  <?php endif ?>
						</div>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</section>
<!--menu-ended-->

<?= $this->endSection() ?>
<!-- end -->