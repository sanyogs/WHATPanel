                    ...
                    <table class="cart-table">
                        ...
                        <tbody id="domains">
                                ...
                                foreach($cart as $row) {  
                                   $cart_id = $row->cart_id;
									if (strpos($row->price, '|') !== false) {
										...
										$price = floatval($priceParts[0]);
									} else {
										// If no '|', convert price to float
                            ...
                            <tr>
                                ...
                                <td id="total" style='width: calc(100% / 6);'><strong><?= AppLib::format_currency($total + $tax_total, 'default_currency') ?></strong></td>
                                <td style='width: calc(100% / 6);'><a href="<?=base_url()?>cart/remove_all" class="common-button w-100">Remove All</a></td>
                            </tr>
                            <tr>
                                <td style='width: calc(100% / 6);'></td>
                                <td style='width: calc(100% / 6);'></td>
                                ...
                                <td style='width: calc(100% / 6);'><input type="text" name="discount_code" id="discount_code" class="common-input m-0" style='border: 1px solid #1912d3 !important;'></td>
                ...
                <div class="continue-shopping p-3">
                    <div class="row ">
                        ...
                        <div class="col-sm-6 col-12 text-end">
                            <a onclick="removeSession()" href="<?= base_url('cart/checkout') ?>" id="submitOrder">
                                <span>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="23" height="16" viewBox="0 0 23 16" fill="none">
                                        <path d="M22 9C22.5523 9 23 8.55228 23 8C23 7.44772 22.5523 7 22 7L22 9ZM0.292892 7.2929C-0.0976315 7.68342 -0.0976314 8.31658 0.292893 8.70711L6.65686 15.0711C7.04738 15.4616 7.68054 15.4616 8.07107 15.0711C8.46159 14.6805 8.46159 14.0474 8.07107 13.6569L2.41421 8L8.07107 2.34315C8.46159 1.95262 8.46159 1.31946 8.07107 0.928933C7.68054 0.538409 7.04738 0.538409 6.65685 0.928934L0.292892 7.2929ZM22 7L1 7L1 9L22 9L22 7Z" fill="#172F78" />
                                    </svg>
			...
			<script>
                $(document).ready(function(e) {
                    $('#validate_disc_code').on('click', function(e) {
						...
						// Show alert message
    					alert('Button clicked. Please wait while we validate your discount code.');
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
								...
								{
									$('#promoCodeMessage').html(response.msg);
									$('#validate_disc_code').prop('disabled', false);
								}
                            }
                        });
                    });
