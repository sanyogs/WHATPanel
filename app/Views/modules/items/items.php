...
switch ($view) {
	...
	case 'hosting':
		$path = 'hosting';
		$view_title = 'Hosting Package';
		$button_title = 'Hosting';
		break;
	case 'service':
		$path = 'service';
		$view_title = 'Products & Services';
		$button_title = 'Products & Services';
		break;
}

if (!isset($param)) { ?>
	<a class="btn btn-default btn-sm btn-responsive" href="<?= base_url() ?>items"><?= lang('hd_lang.all_items') ?></a>
	<a class="btn btn-twitter btn-sm btn-responsive" href="<?= base_url() ?>items?view=domains"><?= lang('hd_lang.domains') ?></a>
	<a class="btn btn-success btn-sm btn-responsive" href="<?= base_url() ?>items?view=hosting"><?= lang('hd_lang.hosting') ?></a>
	<a class="btn btn-warning btn-sm btn-responsive" href="<?= base_url() ?>items?view=service"><?= lang('hd_lang.service') ?></a>
<?php }	?>
		...
		<div class="hs-topbar-wrap">
			...
			<div class="hs-search-wrap">
				<div class="hs-InputWrap">
					...
					<form method='get' action="<?php echo base_url('hosting/index/' . $param); ?>" id="searchForm" style="display:flex;">
						<input type="text" name="search" placeholder="Search for Category Name, Server, and Name" style="margin-right: 10px;" />
						<a href="<?php echo base_url('hosting/index/' . $param); ?>" class="btn new-hosting-div bg-danger clrBtn" style="padding: 7px;">Reset</a>
					</form>
				</div>
				<?php if (isset($path)) { ?>
					<div class="row gap-3 px-3">
						<a href="<?= base_url() ?>hosting/add_<?= $path ?>" class="new-hosting-div col-auto" data-toggle="ajaxModal">
						<?php }	?>
						<span>
						...
						<p>New <?= $button_title; ?></p>
						</a>

						<?php if ((isset($view) && $view == 'hosting')) { ?>
	<a href="<?= base_url() ?>hosting/duplicate_<?= $path ?>/<?= $path ?>" class="new-hosting-div col-auto" data-toggle="ajaxModal">
								<span>
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
										<path d="M10.6667 10.3333C10.6667 10.5101 10.5964 10.6797 10.4714 10.8047C10.3464 10.9298 10.1768 11 10 11H8.66667V12.3333C8.66667 12.5101 8.59643 12.6797 8.47141 12.8047C8.34638 12.9298 8.17681 13 8 13C7.82319 13 7.65362 12.9298 7.5286 12.8047C7.40357 12.6797 7.33333 12.5101 7.33333 12.3333V11H6C5.82319 11 5.65362 10.9298 5.5286 10.8047C5.40357 10.6797 5.33333 10.5101 5.33333 10.3333C5.33333 10.1565 5.40357 9.98695 5.5286 9.86193C5.65362 9.73691 5.82319 9.66667 6 9.66667H7.33333V8.33333C7.33333 8.15652 7.40357 7.98695 7.5286 7.86193C7.65362 7.73691 7.82319 7.66667 8 7.66667C8.17681 7.66667 8.34638 7.73691 8.47141 7.86193C8.59643 7.98695 8.66667 8.15652 8.66667 8.33333V9.66667H10C10.1768 9.66667 10.3464 9.73691 10.4714 9.86193C10.5964 9.98695 10.6667 10.1565 10.6667 10.3333ZM16 5.66667V12.3333C15.9989 13.2171 15.6474 14.0643 15.0225 14.6892C14.3976 15.3141 13.5504 15.6656 12.6667 15.6667H3.33333C2.4496 15.6656 1.60237 15.3141 0.97748 14.6892C0.352588 14.0643 0.00105857 13.2171 0 12.3333L0 4.33333C0.00105857 3.4496 0.352588 2.60237 0.97748 1.97748C1.60237 1.35259 2.4496 1.00106 3.33333 1H5.01867C5.32893 1.00026 5.63493 1.07236 5.91267 1.21067L8.01667 2.26667C8.10963 2.31128 8.21155 2.33408 8.31467 2.33333H12.6667C13.5504 2.33439 14.3976 2.68592 15.0225 3.31081C15.6474 3.93571 15.9989 4.78294 16 5.66667ZM1.33333 4.33333V5H14.544C14.4066 4.61139 14.1525 4.27473 13.8165 4.03606C13.4804 3.79739 13.0788 3.66838 12.6667 3.66667H8.31467C8.0044 3.66641 7.6984 3.5943 7.42067 3.456L5.31667 2.40333C5.22398 2.35757 5.12204 2.33362 5.01867 2.33333H3.33333C2.8029 2.33333 2.29419 2.54405 1.91912 2.91912C1.54405 3.29419 1.33333 3.8029 1.33333 4.33333ZM14.6667 12.3333V6.33333H1.33333V12.3333C1.33333 12.8638 1.54405 13.3725 1.91912 13.7475C2.29419 14.1226 2.8029 14.3333 3.33333 14.3333H12.6667C13.1971 14.3333 13.7058 14.1226 14.0809 13.7475C14.456 13.3725 14.6667 12.8638 14.6667 12.3333Z" fill="white" />
									</svg>
								</span>
								<p>Duplicate Hosting</p>
							</a>
		...
		<div class="hs-table-wrap">
			<div class="tableInfoHead">
				<div class="showEntriesWrap">
					<span>Show</span>
					 <form action="<?php echo base_url('hosting/index/' . $view); ?>" method="get">
			...
			<div class="hs-table-overflow table-responsive">
				<table class="hs-table">
					<tr>
						...
						<th>Name</th>
						<?php if ((isset($view) && $view == 'hosting') || !isset($view)) { ?>
							<th>Server</th>
							<th>Package</th>
							<th>Monthly</th>
							<th>3 Months</th>
							...
							<th>Year</th>
						<?php }
						if ((isset($view) && $view == 'domains') || !isset($view)) { ?>
							<th>Registration</th>
							<th>Transfer</th>
							<th>Renewal</th>
						<?php }
						if ((isset($view) && $view == 'service') || !isset($view)) { ?>
							<th>Unit Price</th>
						<?php } ?>
						<th>Order</th>
						<th>Options</th>
					...
					foreach ($items as $key => $item) {
						...
						$currency_amt = json_decode($item['currency_amt'], true);
					...
					?>
						<tr>
							<td>
								<div>
									<?= ($item['cat_name'] == '') ? 'None' : $item['cat_name'] ?>
								</div>
							</td>
							<td><?= $item['item_name'] ?></td>
							<?php 
						
						if ((isset($view) && $view == 'hosting') || !isset($view)) { ?>
							
				<td><?= ($item['server'] != '') ? '<span class="label label-default">' . $item['server'] . '</span>' : '' ?></td>
				
				<td><?= ($item['package_name'] != '') ? '<span class="label label-default">' . $item['package_name'] . '</span>' : '' ?></td>
							
		<td><?= Applib::format_currency($currency_pricing['monthly'] ? $currency_pricing['monthly'] : 0.00, 'default_currency') ?></td>
							
	<td><?= Applib::format_currency($currency_pricing['quarterly'] ? $currency_pricing['quarterly'] : 0.00, 'default_currency') ?></td>
							
<td><?= Applib::format_currency($currency_pricing['semi_annually'] ? $currency_pricing['semi_annually'] : 0.00, 'default_currency') ?></td>
							
	<td><?= Applib::format_currency($currency_pricing['annually'] ? $currency_pricing['annually'] : 0.00, 'default_currency') ?></td>
							
							<?php }
							if ((isset($view) && $view == 'domains') || !isset($view)) { ?>
								<td><?= Applib::format_currency($item['registration'], 'default_currency') ?></td>
								<td><?= Applib::format_currency($item['transfer'], 'default_currency') ?></td>
								<td><?= Applib::format_currency($item['renewal'], 'default_currency') ?></td>
							<?php }
							if ((isset($view) && $view == 'service') || !isset($view)) {?>
								<td><?= Applib::format_currency($item['unit_cost'], 'default_currency') ?></td>
							<?php } ?>
							<td><?= $item['order_by'] ?></td>
							<td>
								<div class="tableIcon">
				<a href="<?= site_url("hosting/edit_" . $path . '/' . $item['item_id']) ?>" data-toggle="ajaxModal" class="edit-link">
										<span>
											<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
									...
									<?php if ($param == 'hosting') { ?>
										<a href="<?= base_url() ?>items/package/<?= $item['item_id'] ?>"  class="edit-link">
											...
											<span class="edit-name">Add Package</span>
										</a>
									<?php }
									if ($param != 'domains') { ?>
						<!-- <a href="<?= base_url() ?>items/item_links/<?= $item['item_id'] ?>" data-toggle="ajaxModal" class="edit-link">
	<span>
		<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
...
<span class="edit-name">Link</span>
</a> -->
										<?php if ($affiliates == 'TRUE') { ?>
					<a href="<?= base_url() ?>items/affiliates/<?= $item['item_id'] ?>" data-toggle="ajaxModal" class="edit-link">
												<span>
													<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
											 ...
											 <span class="edit-name">Affiliates</span>
											</a>
									<?php }
									} ?>
			<a href="<?= site_url("hosting/delete_" . $path . '/' . $item['item_id']) ?>" data-toggle="ajaxModal" class="edit-link">
										<span>
											<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
						...
						<div class="row">
							<?php if (!empty($items)) : ?>
								...
								<?php if ($pager) : ?>
									...
									$pagi_path = 'hosting/index/' . $encoded_param; ?>
