	...
	<div class="container px-0">
		<div class="hs-topbar-wrap">
			...
			<div class="hs-search-wrap">
				<div class="hs-InputWrap">
					...
					<form method='get' action="<?php echo base_url('items/domian_pricing/' . $param); ?>" id="searchForm" style="display:flex";>
						<input type="text" name="search" placeholder="Search for Code" />
			<a href="<?php echo base_url('items/domian_pricing/' . $param); ?>" class="btn new-hosting-div bg-danger clrBtn" style="padding-top: 3px";>Clear</a>
					</form>
				</div>
				<?php if (isset($param)) { ?>
					<a href="<?= site_url("hosting/add_" . $path) ?>" class="new-hosting-div" data-toggle="ajaxModal"><?php } ?>
					<span>
		...
		<div class="hs-table-wrap">
			<div class="tableInfoHead">
				...
				<?php if ($customerpay_mode == 'API') { ?>
				<div class="filterWrap">
					<div class="hs-dateWrap">
						<p class="common-label"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-info-circle" viewBox="0 0 16 16">
								<path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
								<path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0" />
							</svg> Prices are reflected from API</p>
					</div>
				</div>
				<?php } ?>
			</div>
			<div class="hs-table-overflow">
				<table class="hs-table">
					<tr>
						...
						<th><?= lang('hd_lang.order') ?> </th>
						<?php if ($customerpay_mode != 'API') { ?>
							<th><?= lang('hd_lang.options') ?></th>
						<?php } ?>
					</tr>
					<?php foreach ($invoice_items as $key => $item) :
						// Filter the $invoice_items array based on service_type
					...
					?>
						<tr>
							...
							if ($decode->customerpay_mode == 'API') {  ?>
								<td><?= $item->registration_1 ?></td>
								<td><?= $item->transfer_1 ?></td>
								<td><?= $item->renewal_1 ?></td>
							<?php } else {   ?>
								<td><?= Applib::format_currency($item->registration_1, 'default_currency') ?></td>
								<td><?= Applib::format_currency($item->transfer_1, 'default_currency') ?></td>
								<td><?= Applib::format_currency($item->renewal_1, 'default_currency') ?></td>
							<?php } ?>
							<td><?= ($item->ext_order) ? $item->ext_order : 1; ?></td>
							<?php if ($customerpay_mode != 'API') { ?>
								<td>
									<div class="tableIcon">
										<a href="<?= site_url("hosting/edit_" . $path . '/' . $item->id) ?>" data-toggle="ajaxModal">
											<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
										...
										<a href="<?= site_url("items/delete_domains" . '/' . $item->id) ?>" data-toggle="ajaxModal" data-bs-target="#delete-modal-div">
											<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
