...
<table id="table-invoices" class="table table-striped b-t b-light AppendDataTables hs-table">
	...
	switch ($status) {
		...
		<tr>
			...
			<td class="">
				<span class="label label-<?=$label2?>"><?=lang($status)?> <?php if($inv->emailed == 'Yes') { ?><i
																												  class="fa fa-envelope-o"></i><?php } ?></span>
			...
			<td  >
				...
				<a class="btn btn-xs btn-primary d-flex gap-2 align-items-center text-light" href="<?=base_url()?>invoices/view/<?=$inv->inv_id?>"
				...
				<a class="btn btn-xs btn-twitter d-flex gap-2 align-items-center text-dark" href="<?=base_url()?>invoices/edit/<?=$inv->inv_id?>"
				    data-original-title="<?= lang('hd_lang.edit') ?>" >
					<i class="fa fa-pencil"></i>
				</a>

				<?php } ?>

				<a class="btn btn-xs btn-warning  d-flex gap-2 align-items-center text-dark" href="<?=base_url()?>invoices/transactions/<?=$inv->inv_id?>"
				    data-original-title="<?= lang('hd_lang.payments') ?>" >
					<i class="fa fa-money"></i></a>


				<?php if(User::is_admin() || User::perm_allowed(User::get_id(),'email_invoices')) { ?>
				<a class="btn btn-xs btn-success d-flex gap-2 align-items-center text-light" href="<?=base_url()?>invoices/send_invoice/<?=$inv->inv_id?>"
				   data-toggle="ajaxModal"><span 
												 data-original-title="<?=lang('hd_lang.email_invoice')?>" >
				...
				<a href="<?=base_url()?>invoices/remind/<?=$inv->inv_id?>" data-toggle="ajaxModal"
				   class="btn btn-xs d-flex gap-2 align-items-center btn-vk text-dark" data-original-title="<?=lang('hd_lang.send_reminder')?>">
					<span  data-original-title="<?=lang('hd_lang.send_reminder')?>"
						  >
						<i class="fa fa-bell"></i></span> </a>
				<?php endif; ?>

				<a class="btn btn-xs d-flex gap-2 align-items-center btn-linkedin text-dark" href="<?=base_url()?>invoices/pdf/<?=$inv->inv_id?>"
				    data-original-title="<?=lang('hd_lang.pdf') ?>" >
					<i class="fa fa-file-pdf-o"></i></a>

				<?php if(User::is_admin() || User::perm_allowed(User::get_id(),'delete')) { ?>

				<a class="btn btn-xs d-flex gap-2 align-items-center btn-danger text-light" href="<?= base_url() ?>invoices/delete/<?= $inv->inv_id ?>"
				   data-toggle="ajaxModal">
					<span  data-original-title="<?=lang('hd_lang.delete')?>" ><i
