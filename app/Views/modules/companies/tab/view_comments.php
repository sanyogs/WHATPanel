        ...
        <section class="comment-list block">
            ...
            echo form_open(base_url() . 'companies/comment', $attributes); ?>
                        ...
                        <textarea class="form-control foeditor-100" name="comment"
                            placeholder="<?php echo $i->company_name; ?> <?= lang('hd_lang.comment') ?>" required></textarea>
                        <footer class="panel-footer bg-light lter">
                            <button class="btn btn-<?= $helper->getconfig_item('theme_color') ?> pull-right btn-sm" type="submit">
                                <i class="fa fa-comments"></i>
                                <?= lang('hd_lang.comment') ?>
                            </button>
        ...
        ?>
            ...
            <article id="comment-id-1" class="comment-item">
                ...
                <section class="comment-body panel panel-default">
                    <header class="panel-heading bg-white">
                        ...
                        <label class="label bg-<?= $role_label ?> m-l-xs"><?= ucfirst(User::get_role($c->posted_by)) ?>
                        </label>
                        <span class="text-muted m-l-sm pull-right">
                            <?php echo humanFormat(strtotime($c->date_posted)) . ' ' . lang('hd_lang.ago'); ?>
                            <?php
                if ($c->posted_by == User::get_id()) { ?>

                            <a href="<?= base_url() ?>companies/comment/<?= $c->comment_id ?>/delete"
                                data-toggle="ajaxModal" title="<?= lang('hd_lang.comment_reply') ?>"><i
                    ...
                    <div class="panel-body">
                        <div class="text-dark activate_links">
                            <?php echo nl2br_except_pre($c->message) ?>
                        </div>
            ...
            <article id="comment-id-1" class="comment-item">
                <section class="comment-body panel panel-default">
                    <div class="panel-body">
                        <p>
                            <?= lang('hd_lang.no_comments_found') ?>
                        </p>
                    </div>
                </section>
            </article>
