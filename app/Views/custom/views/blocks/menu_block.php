<ul <?= ($menu->id == 1) ? 'class="navbar-nav ms-auto"' : 'class="menu-' . $menu->id . '"' ?>>
    <?php
    ...
    for ($i = 0; $i < count($menu->main_menu, true); $i++) {
        ...
        if (count($menu->main_menu[$i]->parent_menu, true) == 0) : ?>
            <li><a class="nav-link dropdown-toggle" href="<?= ($url != '/') ? $url : base_url(); ?>">
                    <?php echo
                    $menu->main_menu[$i]->title ?></a></li>
        <?php else : ?>
            <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" id="navbarDropdown" role="button" aria-expanded="false" href="<?php echo base_url() . $url ?>" data-bs-toggle="dropdown"><?php
