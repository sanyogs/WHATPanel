...
class Affiliates extends WhatPanel 
{
    ...
    function index()
    {
        $this->template->title(lang('hd_lang.affiliates'));
        $data['page'] = lang('hd_lang.affiliates'); 
    ...
    function pay_withdrawal($id = null)
    {
        ...
        { 
            ...
            $this->session->set_flashdata('message', lang('hd_lang.payment_recorded'));				
    ...
    {   
        ...
        if($company->affiliate_balance < $this->input->post('amount'))
        {
            ...
            $this->session->set_flashdata('message', lang('hd_lang.invalid_withdrawal_amount'));				
            redirect($_SERVER['HTTP_REFERER']);
        }
        else
        {   
            ...
            {          
                ...
                $this->session->set_flashdata('response_status', 'success');
                $this->session->set_flashdata('message', lang('hd_lang.withdrawal_request_submitted'));				
                redirect($_SERVER['HTTP_REFERER']);
            }
    ...
    function config()
    {
        ...
        {  
            ...
            $this->session->set_flashdata('response_status', 'success');
            $this->session->set_flashdata('message', lang('hd_lang.settings_saved'));				
            redirect($_SERVER['HTTP_REFERER']);           
        }        
    } 
