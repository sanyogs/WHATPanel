...
class MigrationRunner
{
    ...
    public function latest(?string $group = null)
    {
        ...
        foreach ($migrations as $migration) {
            if ($this->migrate('up', $migration)) {
                ...
                $message = lang('hd_lang.Migrations.generalFault');
    ...
    public function regress(int $targetBatch = 0, ?string $group = null)
    {
        ...
        if ($targetBatch !== 0 && ! in_array($targetBatch, $batches, true)) {
            $message = lang('hd_lang.Migrations.batchNotFound') . $targetBatch;
        ...
        while ($batch = array_pop($batches)) {
            ...
            foreach ($this->getBatchHistory($batch, 'desc') as $history) {
                ...
                if (! isset($allMigrations[$uid])) {
                    $message = lang('hd_lang.Migrations.gap') . ' ' . $history->version;
    ...
    public function force(string $path, string $namespace, ?string $group = null)
    {
        ...
        if (empty($migration)) {
            $message = lang('hd_lang.Migrations.notFound');
    ...
    protected function addHistory($migration, int $batch)
    {
        ...
        if (is_cli()) {
            $this->cliMessages[] = sprintf(
                "\t%s(%s) %s_%s",
                CLI::color(lang('hd_lang.Migrations.added'), 'yellow'),
                $migration->namespace,
                $migration->version,
                $migration->class
            );
    ...
    protected function removeHistory($history)
    {
        ...
        if (is_cli()) {
            $this->cliMessages[] = sprintf(
                "\t%s(%s) %s_%s",
                CLI::color(lang('hd_lang.Migrations.removed'), 'yellow'),
                $history->namespace,
                $history->version,
                $history->class
            );
    ...
    protected function migrate($direction, $migration): bool
    {
        ...
        if (! class_exists($class, false)) {
            $message = sprintf(lang('hd_lang.Migrations.classNotFound'), $class);
        ...
        if (! is_callable([$instance, $direction])) {
            $message = sprintf(lang('hd_lang.Migrations.missingMethod'), $direction);
