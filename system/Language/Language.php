...
class Language
{
    ...
    protected function formatMessage($message, array $args = [])
    {
        ...
        if ($formatted === false) {
            throw new InvalidArgumentException(
                lang('hd_lang.Language.invalidMessageFormat', [$message, implode(',', $args)])
            );
        }
    ...
    public function load(string $file, string $locale, bool $return = false)
    {
        ...
        //$path = ROOTPATH."system/Language/{$locale}/{$file}.php";
		
		if($file == 'hd_lang' || $file == 'tank_auth_lang')
		{
			$path = "Language/{$locale}/{$file}.php";
		}
		else
		{
			$path = "Language/{$locale}/{$file}";
		}
