...
trait TimeTrait
{
    ...
    public function humanize()
    {
        ...
        if ($years !== 0) {
            $phrase = lang('hd_lang.Time.years', [abs($years)]);
            $before = $years < 0;
        } elseif ($months !== 0) {
            $phrase = lang('hd_lang.Time.months', [abs($months)]);
            $before = $months < 0;
        } elseif ($days !== 0 && (abs($days) >= 7)) {
            $weeks  = ceil($days / 7);
            $phrase = lang('hd_lang.Time.weeks', [abs($weeks)]);
            $before = $days < 0;
        } elseif ($days !== 0) {
            $before = $days < 0;
            ...
            if (abs($days) === 1) {
                return $before ? lang('hd_lang.Time.yesterday') : lang('hd_lang.Time.tomorrow');
            }

            $phrase = lang('hd_lang.Time.days', [abs($days)]);
        } elseif ($hours !== 0) {
            $phrase = lang('hd_lang.Time.hours', [abs($hours)]);
            $before = $hours < 0;
        } elseif ($minutes !== 0) {
            $phrase = lang('hd_lang.Time.minutes', [abs($minutes)]);
            $before = $minutes < 0;
        } else {
            return lang('hd_lang.Time.now');
        }

        return $before ? lang('hd_lang.Time.ago', [$phrase]) : lang('hd_lang.Time.inFuture', [$phrase]);
    }
