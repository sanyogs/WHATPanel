...
class Publish extends BaseCommand
{
    ...
    public function run(array $params)
    {
        ...
        if ([] === $publishers = Publisher::discover($directory)) {
            CLI::write(lang('hd_lang.Publisher.publishMissing', [$directory]));
        ...
        foreach ($publishers as $publisher) {
            if ($publisher->publish()) {
                CLI::write(lang('hd_lang.Publisher.publishSuccess', [
                    get_class($publisher),
                    count($publisher->getPublished()),
                    $publisher->getDestination(),
                ]), 'green');
            } else {
                CLI::error(lang('hd_lang.Publisher.publishFailure', [
                    get_class($publisher),
                    $publisher->getDestination(),
                ]), 'light_gray', 'red');
