...
class UploadedFile extends File implements UploadedFileInterface
{
    ...
    {
        $errors = [
            UPLOAD_ERR_OK         => lang('hd_lang.HTTP.uploadErrOk'),
            UPLOAD_ERR_INI_SIZE   => lang('hd_lang.HTTP.uploadErrIniSize'),
            UPLOAD_ERR_FORM_SIZE  => lang('hd_lang.HTTP.uploadErrFormSize'),
            UPLOAD_ERR_PARTIAL    => lang('hd_lang.HTTP.uploadErrPartial'),
            UPLOAD_ERR_NO_FILE    => lang('hd_lang.HTTP.uploadErrNoFile'),
            UPLOAD_ERR_CANT_WRITE => lang('hd_lang.HTTP.uploadErrCantWrite'),
            UPLOAD_ERR_NO_TMP_DIR => lang('hd_lang.HTTP.uploadErrNoTmpDir'),
            UPLOAD_ERR_EXTENSION  => lang('hd_lang.HTTP.uploadErrExtension'),
        ];

        $error = $this->error ?? UPLOAD_ERR_OK;

        return sprintf($errors[$error] ?? lang('hd_lang.HTTP.uploadErrUnknown'), $this->getName());
    }
