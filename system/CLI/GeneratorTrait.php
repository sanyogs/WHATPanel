...
trait GeneratorTrait
{
    ...
    private function generateFile(string $target, string $content): void
    {
        if ($this->getOption('namespace') === 'CodeIgniter') {
            ...
            CLI::write(lang('hd_lang.CLI.generator.usingCINamespace'), 'yellow');
            CLI::newLine();

            if (CLI::prompt('Are you sure you want to continue?', ['y', 'n'], 'required') === 'n') {
                CLI::newLine();
                CLI::write(lang('hd_lang.CLI.generator.cancelOperation'), 'yellow');
                CLI::newLine();
        ...
        if (! $this->getOption('force') && $isFile) {
            CLI::error(lang('hd_lang.CLI.generator.fileExist', [clean_path($target)]), 'light_gray', 'red');
            CLI::newLine();
        ...
        if (! write_file($target, $content)) {
            // @codeCoverageIgnoreStart
            CLI::error(lang('hd_lang.CLI.generator.fileError', [clean_path($target)]), 'light_gray', 'red');
            CLI::newLine();
        ...
        if ($this->getOption('force') && $isFile) {
            CLI::write(lang('hd_lang.CLI.generator.fileOverwrite', [clean_path($target)]), 'yellow');
            CLI::newLine();

            return;
        }

        CLI::write(lang('hd_lang.CLI.generator.fileCreate', [clean_path($target)]), 'green');
        CLI::newLine();
    }
    ...
    protected function buildPath(string $class): string
    {
        ...
        if (! $base = reset($base)) {
            CLI::error(lang('hd_lang.CLI.namespaceNotDefined', [$namespace]), 'light_gray', 'red');
            CLI::newLine();
