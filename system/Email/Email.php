...
class Email
{
    ...
    public function attach($file, $disposition = '', $newname = null, $mime = '')
    {
        if ($mime === '') {
            if (strpos($file, '://') === false && ! is_file($file)) {
                $this->setErrorMessage(lang('hd_lang.Email.attachmentMissing', [$file]));

                return false;
            }

            if (! $fp = @fopen($file, 'rb')) {
                $this->setErrorMessage(lang('hd_lang.Email.attachmentUnreadable', [$file]));
    ...
    {
        if (! is_array($email)) {
            $this->setErrorMessage(lang('hd_lang.Email.mustBeArray'));
        ...
        foreach ($email as $val) {
            if (! $this->isValidEmail($val)) {
                $this->setErrorMessage(lang('hd_lang.Email.invalidAddress', [$val]));
    ...
    public function send($autoClear = true)
    {
        ...
        if (! isset($this->headers['From'])) {
            $this->setErrorMessage(lang('hd_lang.Email.noFrom'));
        ...
        if (
            ...
            $this->setErrorMessage(lang('hd_lang.Email.noRecipients'));
    ...
    protected function spoolEmail()
    {
        ...
        if (! $success) {
            $message = lang('hd_lang.Email.sendFailure' . ($protocol === 'mail' ? 'PHPMail' : ucfirst($protocol)));
        ...
        $this->setErrorMessage(lang('hd_lang.Email.sent', [$protocol]));
    ...
    protected function sendWithSendmail()
    {
        ...
        if ($status !== 0) {
            $this->setErrorMessage(lang('hd_lang.Email.exitStatus', [$status]));
            $this->setErrorMessage(lang('hd_lang.Email.noSocket'));
    ...
    {
        if ($this->SMTPHost === '') {
            $this->setErrorMessage(lang('hd_lang.Email.noHostname'));
        ...
        if (strpos($reply, '250') !== 0) {
            $this->setErrorMessage(lang('hd_lang.Email.SMTPError', [$reply]));
    ...
    protected function SMTPConnect()
    {
        ...
        if (! is_resource($this->SMTPConnect)) {
            $this->setErrorMessage(lang('hd_lang.Email.SMTPError', [$errno . ' ' . $errstr]));
        ...
        if ($this->SMTPCrypto === 'tls') {
            ...
            if ($crypto !== true) {
                $this->setErrorMessage(lang('hd_lang.Email.SMTPError', [$this->getSMTPData()]));
    ...
    protected function SMTPAuthenticate()
    {
        ...
        if ($this->SMTPUser === '' && $this->SMTPPass === '') {
            $this->setErrorMessage(lang('hd_lang.Email.noSMTPAuth'));
        ...
        if (strpos($reply, '334') !== 0) {
            $this->setErrorMessage(lang('hd_lang.Email.failedSMTPLogin', [$reply]));
        ...
        if (strpos($reply, '334') !== 0) {
            $this->setErrorMessage(lang('hd_lang.Email.SMTPAuthUsername', [$reply]));
        ...
        if (strpos($reply, '235') !== 0) {
            $this->setErrorMessage(lang('hd_lang.Email.SMTPAuthPassword', [$reply]));
    ...
    protected function sendData($data)
    {
        ...
        if (! is_int($result)) {
            $this->setErrorMessage(lang('hd_lang.Email.SMTPDataFailure', [$data]));
