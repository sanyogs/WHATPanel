...
use Modules\plesk\libraries\Plesk_exec;

class Plesk extends WhatPanel
{
    public function plesk_config($values = null)
    {
        $result = new Plesk_WHMCS();
        $packages = $result->plesk_PackageName($values);
    ...
    public function create_account($params)
    {
        ...
        $plesk_exec = new Plesk_exec($params->server);
    ...
    public function get_usage($order)
    {
        ...
        if (isset($res['limits']['limit'])) {
            foreach ($res['limits']['limit'] as $limit) {
                ...
                if ($limit['name'] == 'disk_space') {
                    if ($limit['value'] > 0) {
                        $usage['disk_limit'] = $limit['value'] / (1024 * 1024);
                    }
                }

                if ($limit['name'] == 'max_traffic') {
                    if ($limit['value'] > 0) {
                        $usage['bw_limit'] = $limit['value'] / (1024 * 1024);
                    }
                }
            }
        }
        ...
        if ($order->reseller_package == 'Yes') {
            ...
            if ($res['stat']['disk-space'] > 0) {
                $disk_usage = $res['stat']['disk-space'] / (1024 * 1024);
            }
            if ($res['stat']['traffic'] > 0) {
                $usage['bw_usage'] = $res['stat']['traffic'] / (1024 * 1024);
            }
            if (isset($res['stat']['traffic']) && $bw_limit == 0) {
                $usage['bw_limit'] = 9999999999;
            }
        } else {
            ...
            if (isset($res['resource-usage']['resource'])) {
                foreach ($res['resource-usage']['resource'] as $used) {
                    ...
                    if ($used['name'] == 'disk_space') {
                        if ($used['value'] > 0) {
                            $usage['disk_usage'] = $used['value'] / (1024 * 1024);
                        }
                    }

                    if ($used['name'] == 'max_traffic') {
                        if ($used['value'] > 0) {
                            $usage['bw_usage'] = $used['value'] / (1024 * 1024);
                        }
                    }
                }
            }
    ...
    public function getAccounts($server)
    {
        $plesk_exec = new Plesk_exec($server);
    ...
    public function import($server)
    {
        try {
            ...
            // $this->load->library('plesk/Plesk_exec', $server);
            $response = $this->getAccounts($server);
    ...
    {
        $code = '<a href="' . base_url() . 'accounts/view_logins/' . $id . '" class="btn btn-sm btn-success common-button" data-toggle="ajaxModal">
        <i class="fa fa-eye"></i>' . lang('hd_lang.view_cpanel_logins') . '</a>
        <a href="' . base_url() . 'accounts/change_password/' . $id . '" class="btn btn-sm btn-info common-button" data-toggle="ajaxModal">
        <i class="fa fa-edit"></i>' . lang('hd_lang.change_cpanel_password') . '</a>         
        <a href="' . base_url() . 'accounts/login/' . $id . '" class="btn btn-sm btn-warning common-button" target="_blank"><i class="fa fa-sign-in"></i> &nbsp;' . lang('hd_lang.control_panel') . '</a>';
    ...
    {	
        // Assuming $server is an object with username property
        $username = $server->username;

        // Example usage of session service
        $session = session();

        // Create session data (adjust this based on your session handling)
        $session->set('username', $username);
        $session->set('server', $server);

        // Example of setting flashdata
        $session->setFlashdata('response_status', 'warning');
        $session->setFlashdata('message', 'Your message here'); // Adjust this message as needed
    ...
    {
        $code = '<a class="btn btn-success btn-xs" href="' . base_url() . 'servers/index/' . $server->id . '" style= "color:white !important;"><i class="fa fa-options"></i> ' . lang('hd_lang.test_connection') . '</a>
        <a class="btn btn-warning btn-xs" href="' . base_url() . 'servers/import/' . $server->id . '" style= "color:white !important;"><i class="fa fa-download"></i> ' . lang('hd_lang.import_accounts') . '</a>
        <a class="btn btn-primary btn-xs" href="' . base_url() . 'servers/edit_server/' . $server->id . '" data-toggle="ajaxModal" style= "color:white !important;"><i class="fa fa-pencil"></i> ' . lang('hd_lang.edit') . '</a>
        <a class="btn btn-danger btn-xs" href="' . base_url() . 'servers/delete_server/' . $server->id . '" data-toggle="ajaxModal" style= "color:white !important;"><i class="fa fa-trash"></i> ' . lang('hd_lang.delete') . '</a>
        <a class="btn btn-success btn-xs" href="' . base_url() . 'servers/login/' . $server->id . '" target="_blank" style= "color:white !important;" ><i class="fa fa-user"></i> ' . lang('hd_lang.login') . '</a>';
        return $code;
    }
