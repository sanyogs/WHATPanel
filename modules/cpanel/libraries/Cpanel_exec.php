...
namespace Modules\cpanel\libraries;
...
class Cpanel_exec 
{
    ...
    {   
        ...
        if (empty($options->username)) {
            $helper->flash_error('Username is not set');
			redirect()->to('servers')->with('Username is not set', 2301);
        }
        if (empty($options->authkey)) {
            $helper->flash_error('Hash is not set');
			redirect()->to('servers')->with('Hash is not set', 2302);
        }
        if (empty($options->hostname)) {
            $helper->flash_error('CPanel Host is not set');
			redirect()->to('servers')->with('CPanel Host is not set', 2303);
        }
